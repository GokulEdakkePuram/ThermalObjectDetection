...
def link_or_copy(src: Path, dst: Path, *, copy: bool) -> None:
    ...
    if copy:
        ...
        # Relative symlink so the dataset survives the repo being moved.
...
def write_data_yaml(config: Path, out: Path, classes: list[str], splits: list[str]) -> None:
    ...
    # Portable, repo-relative path (Ultralytics resolves it against the CWD, so run
    # the scripts from the repo root). Falls back to absolute if `out` is elsewhere.
    try:
        path_str = os.path.relpath(out.resolve(), Path.cwd())
    except ValueError:
    ...
    lines = [
        ...
        "# `path` is relative to the repo root; run scripts from there.",
        f"path: {path_str}",
        "train: images/train",
        "val: images/val",
    ]
